...
def compute_overlookedness_posterior(enriched: pd.DataFrame) -> pd.DataFrame:
    ...
    # Candidate pool: countries with an observed per_pin_gap — i.e., those
    # with both a formal HNO filed and positive FTS requirements. This is
    # the population a full HRP cycle is conducted for, and the population
    # CERF Underfunded Emergencies itself largely picks from.
    #
    # Attempts to expand (requirements>0 any-appeal gate, or completeness
    # floor) either bring back the non-HRP pathology (CHL/TTO/AGO dominate
    # via mechanically-saturated coverage_shortfall) or crowd out
    # fully-observed CERF-aligned picks (SOM, NER, HTI, TCD) in favour of
    # partial-data countries, degrading external precision@k. The
    # 22-country gate is the one the data supports.
    pool = enriched[enriched["per_pin_gap"].notna()].copy()
    if len(pool) == 0:
        return out

    inputs = prepare_inputs(pool)
