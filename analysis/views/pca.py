...
def render(enriched: pd.DataFrame, lens, registry) -> None:
    ...
    # Drop zero-variance columns (constants after drop-na) — they have no
    # direction in PC space and trip divide-by-zero warnings in sklearn.
    stds = X.std(ddof=0)
    zero_var = stds[stds == 0].index.tolist()
    if zero_var:
        st.caption(
            f"Dropped zero-variance on this pool: {', '.join(f'`{c}`' for c in zero_var)}."
        )
        X = X.drop(columns=zero_var)
        cols = [c for c in cols if c not in zero_var]
    if X.shape[1] < 4:
        st.info(
            f"After dropping zero-variance columns, only {X.shape[1]} properties remain. "
            "PCA is suppressed below 4."
        )
        return
