...
from _theme import apply_theme, back_to_landing, page_header, COLORS  # noqa: E402
...
def page_overview() -> None:
    st.title("From data to attributes to posterior")
    st.caption(
        "Six attributes, derived from five upstream sources plus one EU-JRC panel, "
        "feed a Bayesian latent-variable model over humanitarian overlookedness. "
        "This page traces each attribute back to its source and shows the candidate "
        "pool the model fits."
    )
    ...
    pop = load_population()
    inf_latest = load_inform()

    # ── Attribute-to-source provenance table ───────────────────────────
    st.markdown("### The six attributes and where they come from")

    # Per-attribute 2025 availability — count of countries with non-null.
    def _n_with(col_series) -> int:
        return int(col_series.notna().sum())

    # a1 coverage_shortfall: FTS requirements > 0 implies coverage defined
    fts25 = fts[fts["year"] == 2025]
    a1_n = int((fts25["requirements"].fillna(0) > 0).sum())
    # a2 per_pin_gap: needs FTS requirements AND HNO PIN — this is the 22-count gate
    hno25_iso = set(hno25["Country ISO3"].dropna().astype(str).str.upper())
    fts25_iso = set(fts25[fts25["requirements"].fillna(0) > 0]["countryCode"].dropna().astype(str).str.upper())
    a2_n = len(hno25_iso & fts25_iso)
    # a3 need_intensity: needs HNO PIN AND CoD-PS population
    pop_iso = set(pop["ISO3"].dropna().astype(str).str.upper())
    a3_n = len(hno25_iso & pop_iso)
    # a4 severity_category: any country with an INFORM category value
    if not inf_latest.empty and "category" in inf_latest.columns:
        a4_n = int(inf_latest.dropna(subset=["category"])["ISO3"].nunique())
    else:
        a4_n = 0
    # a5 donor_hhi: countries with at least 2 distinct donors in 2025
    inc25 = incoming[incoming["budgetYear"] == 2025].copy()
    inc25["iso3"] = inc25["destLocations"].astype(str).str.strip().str.upper()
    a5_iso = (
        inc25[inc25["iso3"].str.len() == 3]
        .groupby("iso3")["srcOrganization"]
        .nunique()
    )
    a5_n = int((a5_iso >= 2).sum())
    # a6 cluster_gini: countries with at least 2 clusters in 2025
    cl25 = fts_cl[fts_cl["year"] == 2025].copy()
    cluster_col = "cluster" if "cluster" in cl25.columns else ("clusterCode" if "clusterCode" in cl25.columns else None)
    if cluster_col is not None:
        a6_iso = (
            cl25[cl25["requirements"].fillna(0) > 0]
            .groupby("countryCode")[cluster_col]
            .nunique()
        )
        a6_n = int((a6_iso >= 2).sum())
    else:
        a6_n = 0

    prov = pd.DataFrame(
        [
            {"Attribute": "a₁  coverage shortfall",
             "Support": "[0, 1]",  "Likelihood": "Beta regression",
             "Source(s)": "FTS requirements & funding",
             "2025 countries with value": a1_n},
            {"Attribute": "a₂  gap per person in need",
             "Support": "(0, ∞)", "Likelihood": "log-normal",
             "Source(s)": "FTS requirements/funding × HNO PIN",
             "2025 countries with value": a2_n},
            {"Attribute": "a₃  need intensity",
             "Support": "[0, 1]", "Likelihood": "Beta regression",
             "Source(s)": "HNO PIN ÷ CoD-PS population",
             "2025 countries with value": a3_n},
            {"Attribute": "a₄  severity (INFORM)",
             "Support": "{1..5}", "Likelihood": "ordered logistic",
             "Source(s)": "INFORM Severity (EU JRC)",
             "2025 countries with value": a4_n},
            {"Attribute": "a₅  donor concentration",
             "Support": "[0, 1]", "Likelihood": "Beta regression",
             "Source(s)": "FTS incoming flows (HHI over donors)",
             "2025 countries with value": a5_n},
            {"Attribute": "a₆  intra-crisis equity",
             "Support": "[0, 1]", "Likelihood": "Beta regression",
             "Source(s)": "FTS cluster-level coverage (PIN-weighted Gini)",
             "2025 countries with value": a6_n},
        ]
    )
    st.dataframe(
        prov,
        ...
        hide_index=True,
        column_config={
            "2025 countries with value": st.column_config.ProgressColumn(
                "2025 countries with value",
                format="%d",
                min_value=0,
                max_value=max(a1_n, a2_n, a3_n, a4_n, a5_n, a6_n, 1),
            ),
        },
    )

    # ── Candidate pool callout ─────────────────────────────────────────
    st.markdown("### The candidate pool")

    pool_iso = sorted(hno25_iso & fts25_iso)
    c1, c2 = st.columns([1, 2])
    with c1:
        st.metric(
            f"HRP-eligible for 2025",
            f"{len(pool_iso)} countries",
            help=(
                "Countries with BOTH an HNO people-in-need figure AND positive FTS "
                "requirements in 2025. This is the pool the Bayesian model fits, "
                "and largely the pool CERF UFE itself picks from."
            ),
        )
        st.caption(
            "The pool is gated by `per_pin_gap` observed — the only attribute that "
            "requires both an HNO and an FTS row. Countries with only one of the two "
            "(e.g., FTS Flash Appeal but no HNO filed) fall out."
        )
    with c2:
        st.markdown("**The 22 countries fitted in the 2025 cycle:**")
        st.code(" · ".join(pool_iso), language=None)

    # ── Top-line 2025 figures ──────────────────────────────────────────
    st.markdown("### 2025 top-line figures")
    ...
    st.markdown("---")
    st.caption(
        "**Other tabs** — Cross-dataset coverage (the data-join; which countries appear "
        "in which source), Needs / Funding / Severity / Pooled funds / Plans (each source "
        "per-attribute). The Bayesian posterior, four-cell typology, and external-benchmark "
        "validation all live on the semantic-analysis surface at http://localhost:8502."
    )
...
def page_coverage() -> None:
    ...
    st.caption(
        "The data join that defines the candidate pool. A country enters the "
        "Bayesian fit if it has BOTH an HNO people-in-need figure AND positive "
        "FTS requirements in the same year — because `per_pin_gap = (req − "
        "funding) ÷ PIN` is the only attribute that requires both."
    )
    ...
    # ── HRP-eligible pool — the explicit data-join metric ─────────────
    hrp_eligible = sets.get("HNO", set()) & sets.get("FTS (country)", set())

    st.markdown("### HRP-eligible pool")
    c1, c2 = st.columns([1, 2])
    with c1:
        st.metric(
            f"With per_pin_gap observed ({year})",
            f"{len(hrp_eligible)} countries",
            help=(
                "Countries with both HNO PIN and FTS requirements. "
                "This is the pool the Bayesian model fits; CERF UFE allocations "
    ...
    with c2:
        ...
        st.markdown(
            f"**HNO ∩ FTS (country) = {len(hrp_eligible)}** · "
            f"HNO ∩ FTS (country) ∩ FTS (cluster) ∩ HRP = **{len(core_inter)}** "
            f"(all four sources aligned — the strictest join)."
        )
        if hrp_eligible:
            st.code(" · ".join(sorted(hrp_eligible)), language=None)
    ...
    mem["HRP-eligible"] = mem.index.isin(hrp_eligible)
    mem["count"] = mem.drop(columns=["HRP-eligible"]).sum(axis=1)
    st.markdown("### Membership matrix")
    st.dataframe(
        mem.sort_values(["HRP-eligible", "count"], ascending=[False, False]).reset_index(),
        use_container_width=True,
        height=420,
    )

    # ── Choropleth — three colour tiers ────────────────────────────────
    st.markdown("### HRP-eligible countries on the map")
    choro_df = pd.DataFrame({"iso3": sorted(all_iso)})
    def _tier(iso: str) -> str:
        if iso in hrp_eligible: return "HRP-eligible"
        if iso in sets.get("HNO", set()): return "HNO only"
        if iso in sets.get("FTS (country)", set()): return "FTS only"
        return "Other source"
    choro_df["tier"] = choro_df["iso3"].map(_tier)
    fig = px.choropleth(
        choro_df,
        locations="iso3",
        locationmode="ISO-3",
        color="tier",
        category_orders={"tier": ["HRP-eligible", "HNO only", "FTS only", "Other source"]},
        color_discrete_map={
            "HRP-eligible": COLORS["accent"],
            "HNO only":     COLORS["accent_soft"],
            "FTS only":     "#cfa59a",
            "Other source": COLORS["faint"],
        },
        title=f"{year}: HRP-eligible countries (both HNO and FTS observed)",
    )
...
def page_hno() -> None:
    st.title("Needs — HNO")
    st.caption(
        "People in Need by country × sector, from OCHA HPC HNO. Feeds two model "
        "attributes: **a₂ gap per PIN** (via the FTS join) and **a₃ need intensity** "
        "(PIN ÷ CoD-PS population). Missing HNO is what excludes ETH and SYR from "
        "the 2025 candidate pool — they have FTS operations but no HNO filed."
    )

    year = st.selectbox("Year", [2025, 2024], index=0)
    df = load_hno(year)
    country_pin = hno_country_pin(year)
    pop = load_population().rename(columns={"ISO3": "iso3", "Population": "population"})
    pop = pop.groupby("iso3", as_index=False)["population"].max()
    joined = country_pin.merge(pop, on="iso3", how="left")
    joined["need_intensity"] = (joined["pin"] / joined["population"]).clip(upper=1.0)

    m1, m2, m3 = st.columns(3)
    m1.metric(f"Total PIN {year}", f"{country_pin['pin'].sum()/1e6:,.1f} M")
    m2.metric("Countries with PIN filed", f"{country_pin['iso3'].nunique()}")
    m3.metric(
        "Population-joined",
        f"{joined['need_intensity'].notna().sum()}",
        help="Countries where need_intensity = PIN ÷ population is computable.",
    )

    # ── a3 need_intensity (left) vs a2 ingredient PIN (right) ─────────
    st.markdown("### a₂ & a₃ ingredients")
    colA, colB = st.columns([1, 1])
    with colA:
        st.markdown("**Need intensity · a₃ = PIN ÷ population**")
        fig3 = px.choropleth(
            joined.dropna(subset=["need_intensity"]),
            locations="iso3",
            locationmode="ISO-3",
            color="need_intensity",
        ...
        fig3.update_layout(
            ...
            height=380,
            coloraxis_colorbar={"title": "a₃"},
        )
        st.plotly_chart(fig3, use_container_width=True)
    with colB:
        st.markdown("**PIN · absolute people in need (a₂ numerator)**")
        fig2 = px.choropleth(
            country_pin,
            locations="iso3",
            locationmode="ISO-3",
        ...
        fig2.update_layout(
            margin={"l": 0, "r": 0, "t": 10, "b": 0},
            height=380,
            coloraxis_colorbar={"title": "PIN"},
        )
        st.plotly_chart(fig2, use_container_width=True)

    # ── Top-20 PIN + cluster drilldown ─────────────────────────────────
    ...
    with colB:
        st.markdown("#### PIN by cluster · selected country")
        ...
        by_cluster = (
            sub.groupby("Cluster", as_index=False)["In Need"]
            .sum().sort_values("In Need", ascending=False)
        )
        st.plotly_chart(
...
def page_fts() -> None:
    ...
    st.caption(
        "Requirements and funding by country × year, from OCHA FTS. The primary "
        "source for **a₁ coverage shortfall** = 1 − min(funding ÷ requirements, 1). "
        "FTS × HNO together define **a₂ gap per PIN**; FTS incoming flows define "
        "**a₅ donor HHI**; FTS cluster-level feeds **a₆ sectoral equity**."
    )
    ...
    sub["a1_coverage_shortfall"] = (1 - sub["coverage"].clip(upper=1)).clip(lower=0)
    ...
    # ── a1 coverage shortfall (map) ─────────────────────────────────────
    st.markdown("### a₁ coverage shortfall · 1 − min(fund/req, 1)")
    fig = px.choropleth(
        sub,
        locations="countryCode",
        locationmode="ISO-3",
        color="a1_coverage_shortfall",
        color_continuous_scale="Reds",
        range_color=(0, 1),
        hover_data=["requirements", "funding", "coverage"],
    )
    fig.update_layout(margin={"l": 0, "r": 0, "t": 10, "b": 0}, height=420,
                      coloraxis_colorbar={"title": "a₁"})
    st.plotly_chart(fig, use_container_width=True)

    # ── a2 per_pin_gap (FTS × HNO join) ─────────────────────────────────
    st.markdown("### a₂ gap per PIN · (req − funding) ÷ HNO PIN")
    st.caption(
        "This attribute requires both FTS requirements and HNO PIN for the same "
        "year. Countries missing either side are not in the candidate pool."
    )
    try:
        hno = load_hno(year)
        pin = (
            hno[hno["Country ISO3"].notna()]
            .groupby("Country ISO3", as_index=False)["In Need"].sum()
            .rename(columns={"Country ISO3": "iso3", "In Need": "pin"})
        )
        j = sub.rename(columns={"countryCode": "iso3"}).merge(pin, on="iso3", how="left")
        j["gap"] = (j["requirements"] - j["funding"]).clip(lower=0)
        j["a2_per_pin_gap"] = j["gap"] / j["pin"].replace({0: np.nan})
        in_pool = j["a2_per_pin_gap"].notna()
    except Exception:
        j = sub.assign(iso3=sub["countryCode"], a2_per_pin_gap=np.nan)
        in_pool = pd.Series(False, index=j.index)

    c1, c2 = st.columns([1, 2])
    with c1:
        st.metric(
            f"In candidate pool ({year})",
            f"{int(in_pool.sum())} countries",
            help="Countries with FTS requirements AND HNO PIN both observed.",
        )
        st.metric(
            "FTS-only (no HNO filed)",
            f"{int(((~in_pool) & (j['requirements'].fillna(0) > 0)).sum())}",
            help="Have FTS operations but no HNO — excluded from the fit (e.g., ETH, SYR in 2025).",
        )
    with c2:
        top = j.dropna(subset=["a2_per_pin_gap"]).nlargest(15, "a2_per_pin_gap")
        if not top.empty:
            fig2 = px.bar(
                top, x="a2_per_pin_gap", y="iso3", orientation="h",
                hover_data=["requirements", "funding", "pin"],
                labels={"a2_per_pin_gap": "a₂ per-PIN gap (USD)"},
            ).update_layout(
    ...
    with colA:
        ...
        st.plotly_chart(
            px.bar(big, x="coverage", y="countryCode", orientation="h",
                ...
                yaxis={"categoryorder": "total descending"}, height=500,
                margin={"l":0,"r":0,"t":10,"b":0}
            ),
            use_container_width=True,
        )
    with colB:
        ...
        if picks:
            ...
            st.plotly_chart(
                ...
                .update_layout(yaxis_tickformat=".0%", height=500,
                               margin={"l":0,"r":0,"t":10,"b":0}),
                use_container_width=True,
            )
...
def page_hrp() -> None:
    ...
    st.caption(
        "The Bayesian model fits countries with **per_pin_gap observed** — both "
        "an HNO people-in-need figure and positive FTS requirements in the same "
        "year. This page shows why the pool is twenty-two in 2025, and which "
        "eight benchmark picks sit outside it."
    )

    hrp = load_hrp()
    year = st.slider("Cycle", 2018, 2026, value=2025, step=1, key="hrp_year")

    # ── HRP source list (formal plans) ─────────────────────────────────
    plans_year = hrp[hrp["years"].fillna("").astype(str).str.contains(str(year))]
    hrp_plan_iso: set[str] = set()
    for s in plans_year["locations"].dropna():
        for tok in str(s).split("|"):
            t = tok.strip().upper()
            if len(t) == 3 and t.isalpha():
                hrp_plan_iso.add(t)

    # ── HRP-eligible pool: intersection of HNO-PIN and FTS-requirements ──
    try:
        hno = load_hno(year)
        hno_iso = set(hno["Country ISO3"].dropna().astype(str).str.upper())
    except Exception:
        hno_iso = set()
    fts = load_fts_global()
    fts_year = fts[fts["year"] == year]
    fts_iso = set(
        fts_year[fts_year["requirements"].fillna(0) > 0]["countryCode"]
        .dropna().astype(str).str.upper()
    )
    eligible = sorted(hno_iso & fts_iso)
    hno_only = sorted(hno_iso - fts_iso)
    fts_only = sorted(fts_iso - hno_iso)

    # ── Header metrics ─────────────────────────────────────────────────
    m1, m2, m3, m4 = st.columns(4)
    m1.metric(f"HRP plans listed ({year})", len(plans_year))
    m2.metric(f"HRP-eligible (HNO ∩ FTS)", f"{len(eligible)}")
    m3.metric("HNO filed, no FTS", f"{len(hno_only)}")
    m4.metric("FTS requirements, no HNO", f"{len(fts_only)}")

    # ── The 22 countries, explicitly named ─────────────────────────────
    st.markdown("### The candidate pool, named")
    if eligible:
        st.code(" · ".join(eligible), language=None)
    else:
        st.info("No countries found with both HNO and FTS data in this cycle.")

    # ── Missing benchmark picks ─────────────────────────────────────────
    st.markdown("### Benchmark picks outside the pool")
    st.caption(
        "Eight countries appear on CERF UFE or CARE BTS but cannot be fitted. "
        "For six of them no formal HRP exists. For ETH and SYR, the HNO 2025 "
        "source file is genuinely empty."
    )
    try:
        bench_path = Path(__file__).resolve().parent.parent / "Data" / "Third-Party" / "Benchmarks"
        cerf = pd.read_csv(bench_path / "cerf_ufe.csv")["iso3"].unique() if (bench_path / "cerf_ufe.csv").exists() else []
        care = pd.read_csv(bench_path / "care_bts.csv")["iso3"].unique() if (bench_path / "care_bts.csv").exists() else []
        benchmark_iso = set(cerf) | set(care)
    except Exception:
        benchmark_iso = set()

    rows = []
    for iso in sorted(benchmark_iso):
        in_pool = iso in eligible
        in_hno = iso in hno_iso
        in_fts = iso in fts_iso
        status = "in pool" if in_pool else (
            "HNO only" if in_hno and not in_fts else (
            "FTS only" if in_fts and not in_hno else "neither")
        )
        on_cerf = iso in set(cerf) if len(cerf) else False
        on_care = iso in set(care) if len(care) else False
        rows.append({
            "ISO3": iso,
            "HNO filed": in_hno,
            "FTS requirements > 0": in_fts,
            "Status": status,
            "On CERF UFE": on_cerf,
            "On CARE BTS": on_care,
        })
    if rows:
        ...
        bench_df = bench_df.sort_values(["Status", "ISO3"])
        st.dataframe(bench_df, use_container_width=True, hide_index=True, height=min(360, 40 + 30 * len(rows)))

    # ── Expander: the raw HRP plan list (formality check) ──────────────
    with st.expander(f"HRP plan metadata for {year} (source: OCHA HPC)"):
        st.dataframe(
            plans_year[
                ["code", "planVersion", "categories", "locations", "years",
...
st.sidebar.markdown("---")
st.sidebar.caption(
    "Analytic views — Bayesian posterior, four-cell typology, external "
    "validation — live on the semantic-analysis surface at "
    "http://localhost:8502.\n\n"
    "Methodology: http://localhost:7777/methodology/"
)
