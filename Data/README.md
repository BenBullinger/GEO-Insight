...
Raw source datasets for Facet. None of the CSV/JSON files in this folder are tracked in git (too large for GitHub; ~270 MB total). The repository ships the **download script** and the **source manifest**; everyone reproduces their local data directory with one command.
